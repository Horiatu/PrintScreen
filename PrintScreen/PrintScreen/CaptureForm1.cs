...
namespace FlexScreen
{
    ...
    public sealed partial class CaptureForm : Form
    {
        ...
        public Rectangle SelectedRectangle
        {
            get
            {
                selectedRectangle.X = Math.Min(StartPoint.X, EndPoint.X);
                selectedRectangle.Y = Math.Min(StartPoint.Y, EndPoint.Y);
                selectedRectangle.Width = Math.Abs(EndPoint.X - StartPoint.X);
                selectedRectangle.Height = Math.Abs(EndPoint.Y - StartPoint.Y);
        ...
        public CaptureForm()
        {
            m_zoomFactor = 1;
            InitializeComponent();
            Cursor = CustomCursors.CrossHair();
            ShowInTaskbar = true;
...
Press [F1] for more help.";
        ...
        ToolStripMenuItem lastMenuItemOpened = null;
        private void FilesToolStripMenuItemDropDownOpened(object sender, EventArgs e)
        {
            lastMenuItemOpened = sender as ToolStripMenuItem;
        ...
        void FormMouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                m_mouseDown = false;
                Capturing = false;
                bool defaultTool = ActiveTool is DefaultTool;
                SetState(CaptureStep.End);
                Cursor.Show();
                if (!IsCroped && Program.MySettings.AutomaticallyCrop && defaultTool && StartPoint.X != EndPoint.X && StartPoint.Y != EndPoint.Y)
                {
                    CropToolStripMenuItemClick(this, new EventArgs());
                }
            }
            else if (e.Button == MouseButtons.Right)
            {
                ...
                if (lastMenuItemOpened != null)
                {
                    rightClickedPos = Cursor.Position;
                    lastMenuItemOpened.ShowDropDown();
                    if (lastMenuClicked.X > 0 && lastMenuClicked.Y > 0)
                    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            e.Graphics.DrawRectangle(new Pen(Color.FromArgb(100, Color.Black)) { Width = 3 }, frame);

            if (m_mode == CaptureStep.Start || m_mode == CaptureStep.End)
            {
            ...
            if (AltKeyPressed)
            {
                if (SelectedRectangle.Width > 0 || SelectedRectangle.Height > 0)
                {
                    e.Graphics.DrawString(string.Format("{0}, {1}", SelectedRectangle.Left, SelectedRectangle.Top),
                        new Font("Arial", 7), new SolidBrush(Program.MySettings.SelectionColor), new Point(Math.Min(StartPoint.X, EndPoint.X) + 1, Math.Min(StartPoint.Y, EndPoint.Y) - 11));
                    e.Graphics.DrawString(string.Format("{0}, {1}", SelectedRectangle.Width, SelectedRectangle.Height),
                        new Font("Arial", 7), new SolidBrush(Program.MySettings.SelectionColor), new Point(EndPoint.X + 1, EndPoint.Y + 1));
                }
            }

            if (m_mouseDown)
            {
                var cursorPen = new Pen(Color.FromArgb(80, Program.MySettings.CursorColor));
                e.Graphics.DrawLine(cursorPen, 0, EndPoint.Y, EndPoint.X - 2, EndPoint.Y);
                e.Graphics.DrawLine(cursorPen, EndPoint.X + 2, EndPoint.Y, ClientRectangle.Width, EndPoint.Y);
                e.Graphics.DrawLine(cursorPen, EndPoint.X, 0, EndPoint.X, EndPoint.Y - 2);
                e.Graphics.DrawLine(cursorPen, EndPoint.X, EndPoint.Y + 2, EndPoint.X, ClientRectangle.Height);
            }
...
{0}",ex.Message), ToolTipIcon.Error);
        ...
        private void DrawingToolStripMenuItemDropDownOpening(object sender, EventArgs e)
        {
            ...
            using (Graphics g = GetMenuIcon(indexToolStripMenuItem))
            {
            ...
            using (Graphics g = GetMenuIcon(pointMarkerToolStripMenuItem))
            {
        ...
        private void exitToolStripMenuItem1_DropDownOpening(object sender, EventArgs e)
        {
        ...
        bool isTransparent = Program.MySettings.TransparentWhenNotFocussed;
